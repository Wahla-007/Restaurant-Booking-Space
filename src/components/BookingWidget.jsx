...
export default function BookingWidget({ restaurant }) {
    ...
    const fetchBookedSlots = async () => {
        ...
        if (!restaurant || !restaurant.id || restaurant.id.toString().length < 20) return;
    ...
    const executeBooking = async (userId, userEmail = null) => {
        ...
        const isStaticRestaurant = !restaurant.id || restaurant.id.toString().length < 20;
    ...
    if (view === 'success') {
        return (
            <div className="glass" style={{ padding: '2rem', borderRadius: 'var(--radius)', textAlign: 'center', boxShadow: 'var(--shadow-lg)', position: 'sticky', top: '100px' }}>
                ...
                {bookingCode && (
                    <div style={{ background: 'rgba(255,255,255,0.1)', padding: '0.5rem 1rem', borderRadius: '8px', margin: '1rem 0', display: 'inline-block' }}>
                        <span style={{ fontSize: '0.8rem', color: 'var(--text-muted)', display: 'block' }}>Booking Reference</span>
                        <code style={{ fontSize: '1.2rem', fontWeight: 'bold', letterSpacing: '1px' }}>{bookingCode}</code>
                    </div>
                )}
